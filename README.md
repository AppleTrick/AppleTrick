# Hi, I'm Chang-hee 👋

> *Making the slow fast, and the complex simple.*

Fullstack developer focused on building user-centered web services.  
Currently finishing up **SSAFY 14th** — shipping AI-integrated products and exploring what LLMs can do in production.

---

## 🛠 Tech Stack

<div align="center">

**Frontend**  
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=fff)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=000)
![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat-square&logo=react&logoColor=61DAFB)

**Backend**  
![Java](https://img.shields.io/badge/Java-007396?style=flat-square&logo=openjdk&logoColor=fff)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=fff)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=fff)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=fff)

**AI / ML**  
![YOLO](https://img.shields.io/badge/YOLO11-00FFFF?style=flat-square&logoColor=000)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=fff)
![LoRA](https://img.shields.io/badge/LoRA_Fine--tuning-FF6F00?style=flat-square&logo=pytorch&logoColor=fff)
![Spring AI](https://img.shields.io/badge/Spring_AI-6DB33F?style=flat-square&logo=spring&logoColor=fff)

**Infra / Tools**  
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=fff)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=fff)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=fff)

</div>

---

## 📂 Projects

<div align="center">

| Project | Description | Stack |
|--------|-------------|-------|
| [**🏠 이집내집**](https://github.com/AppleTrick/ijip-naejip-backend) | NL2SQL with LLM · 1.8M records · query 5s → 1s | Spring AI, GPT, React |
| [**💊 약속 (Yaksok)**](https://github.com/AppleTrick/yaksok) | YOLO11 + OCR pipeline · 75s → 16s · AI supplement management | Spring Boot, FastAPI, Next.js |
| [**💸 싸다구 (Ssadagu)**](https://github.com/AppleTrick/Ssadagu) | P2P fintech · real-time messaging · distributed tx design | Spring Boot, React Native, Turborepo |
| [**🤖 VQA AI Challenge**](https://github.com/AppleTrick/ssafy14-ai-challenge-2025) | LoRA fine-tuning · score 0.237 → 0.916 · **2nd place** | Python, Qwen, LoRA |

</div>

---

## 📊 Stats

<div align="center">

![Solved.ac Profile](http://mazassumnida.wtf/api/v2/generate_badge?boj=akkadia)

</div>


